...
4. 进入 game/ 目录，执行 python manage.py migrate，生成数据表
5. 将 extension/game_demo/ 目录中的
   data_object_creator_types.csv
   data_object_type_list.csv
   data_portable_object_types.csv 拷贝至 game/ 目录中
   （目前这三个文件并不是csv格式的，以后会改成csv格式的。）
6. 进入 game/ 目录，执行 python manage.py dbshell
7. 执行
   .import data_object_creator_types.csv data_object_creator_types
   .import data_object_creator_types.csv data_object_creator_types
   .import data_object_creator_types.csv data_object_creator_types
   将文件中的数据导入数据库
8. 执行
